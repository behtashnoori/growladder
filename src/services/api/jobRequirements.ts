...
export async function get(id: string) {
  const res = await http.get(`/api/job-requirements/${id}`);
...
export async function update(id: string, data: JobRequirement) {
  const res = await http.put(`/api/job-requirements/${id}`, data);
  return res.data as JobRequirement;
}

export async function remove(id: string) {
  const res = await http.delete(`/api/job-requirements/${id}`);
  return res.data as JobRequirement;
}
