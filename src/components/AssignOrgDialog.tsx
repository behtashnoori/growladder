...
const AssignOrgDialog = ({ person, open, onOpenChange }: Props) => {
  ...
  const { data: decrees = [] } = useQuery({ queryKey: ["decrees"], queryFn: () => db.decrees.toArray() });
  ...
  const save = async () => {
    ...
    await db.transaction("rw", db.personnel, db.personOrgHistory, async () => {
      ...
      const newRec: PersonOrgHistory = {
        ...
        decree_code: decree || undefined,
        decree_title: decrees.find((d) => d.code === decree)?.title,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="space-y-3 py-2">
          <Select value={decree} onValueChange={setDecree}>
            <SelectTrigger dir="rtl"><SelectValue placeholder="حکم کارگزینی" /></SelectTrigger>
