...
const AssignOrgDialogFixed = ({ person, open, onOpenChange }: Props) => {
  console.log('🔍 AssignOrgDialogFixed rendered for:', person.emp_code);
  ...
  const { data: postRanksResponse, isLoading: postRanksLoading, error: postRanksError } = useQuery({ 
    queryKey: ["postRanks"], 
  ...
  console.log('📊 Post ranks loading:', postRanksLoading);
  console.log('📊 Post ranks error:', postRanksError);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>تخصیص پرسنل - {person.name} (نسخه جدید)</DialogTitle>
        </DialogHeader>
                  ...
                  <SelectContent dir="rtl">
                    {postRanksLoading ? (
                      ...
                      <SelectItem value="no-data" disabled>
                        هیچ رده پستی یافت نشد
                      </SelectItem>
                    )}
                  </SelectContent>
                </Select>
                <div className="mt-2 text-xs">
                  <p className="text-red-600">📊 تعداد رده پست‌ها: {postRanks.length}</p>
                  <p className="text-red-600">⏳ در حال بارگذاری: {postRanksLoading ? 'بله' : 'خیر'}</p>
                  {postRanksError && <p className="text-red-600">❌ خطا: {postRanksError.message}</p>}
                </div>
              </div>
            </div>
          </div>
...
export default AssignOrgDialogFixed;
