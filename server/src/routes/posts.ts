...
router.get("/:code", async (req, res, next) => {
  try {
    const item = await prisma.post.findUnique({ where: { code: req.params.code } });
...
router.post("/", validate(bodySchema, "body"), async (req, res, next) => {
  try {
    const item = await prisma.post.create({ data: req.body });
...
router.delete("/:code", async (req, res, next) => {
  try {
    await prisma.post.delete({ where: { code: req.params.code } });
    res.status(204).send();
  } catch (e) {
    next(e);
  }
...
router.post("/bulk", validate(bulkSchema, "body"), async (req, res, next) => {
  try {
    ...
    console.log('Bulk create posts:', items.length, 'items');
