...
router.get("/", validate(querySchema, "query"), async (req, res, next) => {
  try {
    ...
    const [total, items] = await Promise.all([
      ...
      prisma.management.findMany({ where, orderBy }),
    ]);
...
router.get("/:code", async (req, res, next) => {
  try {
    const item = await prisma.management.findUnique({ where: { code: req.params.code } });
...
router.post("/", validate(bodySchema, "body"), async (req, res, next) => {
  try {
    const item = await prisma.management.create({ data: req.body });
...
router.put("/:code", validate(bodySchema, "body"), async (req, res, next) => {
  try {
    const item = await prisma.management.update({
...
router.delete("/:code", async (req, res, next) => {
  try {
    await prisma.management.delete({ where: { code: req.params.code } });
    res.status(204).send();
  } catch (e) {
    next(e);
  }
...
router.post("/bulk", validate(bulkSchema, "body"), async (req, res, next) => {
  try {
    ...
    console.log('Bulk create managements:', items.length, 'items');
    const result = await prisma.management.createMany({
      data: items,
      skipDuplicates: true
    });
