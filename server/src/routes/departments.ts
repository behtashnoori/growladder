...
router.get("/", validate(querySchema, "query"), async (req, res, next) => {
  try {
    ...
    const [total, items] = await Promise.all([
      ...
      prisma.department.findMany({ where, orderBy }),
    ]);
...
router.get("/:code", async (req, res, next) => {
  try {
    const item = await prisma.department.findUnique({ where: { code: req.params.code } });
...
router.delete("/:code", async (req, res, next) => {
  try {
    await prisma.department.delete({ where: { code: req.params.code } });
    res.status(204).send();
  } catch (e) {
    next(e);
  }
...
router.post("/bulk", validate(bulkSchema, "body"), async (req, res, next) => {
  try {
    ...
    console.log('Bulk create departments:', items.length, 'items');
