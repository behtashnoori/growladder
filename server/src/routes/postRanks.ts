...
router.get("/", validate(querySchema, "query"), async (req, res, next) => {
  try {
    ...
    const [total, items] = await Promise.all([
      prisma.postRank.count({ where }),
      prisma.postRank.findMany({ where, orderBy }),
    ]);
...
router.get("/:code", async (req, res, next) => {
  try {
    const item = await prisma.postRank.findUnique({ where: { code: req.params.code } });
...
router.post("/", validate(bodySchema, "body"), async (req, res, next) => {
  try {
    const item = await prisma.postRank.create({ data: req.body });
...
router.put("/:code", validate(bodySchema, "body"), async (req, res, next) => {
  try {
    const item = await prisma.postRank.update({
...
router.delete("/:code", async (req, res, next) => {
  try {
    await prisma.postRank.delete({ where: { code: req.params.code } });
    res.status(204).send();
  } catch (e) {
    next(e);
  }
...
router.post("/bulk", validate(bulkSchema, "body"), async (req, res, next) => {
  try {
    ...
    console.log('Bulk create post ranks:', items.length, 'items');
    console.log('Sample item:', items[0]);
    const result = await prisma.postRank.createMany({
      data: items,
      skipDuplicates: true
    });
